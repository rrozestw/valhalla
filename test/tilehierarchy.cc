...
TEST(TileHierarchy, Parse) {
  ...
  EXPECT_EQ((++TileHierarchy::levels().begin())->second.name , "arterial") << "Middle hierarchy should be named arterial";
  EXPECT_EQ(TileHierarchy::levels().begin()->second.level , 0) << "Top hierarchy should have level 0";
  EXPECT_EQ(TileHierarchy::levels().rbegin()->second.tiles.TileSize() , .25f) << "Bottom hierarchy should have tile size of .25f";
  EXPECT_EQ(TileHierarchy::levels().find(5) , TileHierarchy::levels().end()) << "There should only be levels 0, 1, 2";
  EXPECT_NE(TileHierarchy::levels().find(2) , TileHierarchy::levels().end()) << "There should be a level 2";
  ...
  EXPECT_EQ(TileHierarchy::levels().begin()->second.importance , RoadClass::kPrimary) << "Importance should be set to primary";
  EXPECT_EQ((++TileHierarchy::levels().begin())->second.importance , RoadClass::kTertiary) << "Importance should be set to tertiary";
  EXPECT_EQ(TileHierarchy::levels().rbegin()->second.importance , RoadClass::kServiceOther) << "Importance should be set to service/other";
}
