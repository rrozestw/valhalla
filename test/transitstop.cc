...
TEST(TransitStop, TestWriteRead) {
  ...
  EXPECT_THROW(TransitStop transit_stop(kMaxNameOffset + 1, 777, false, trav);, 
               std::runtime_error) << "TransitStop one_stop_offset limit check failed";

  // Test bounds of name offset
  EXPECT_THROW(TransitStop transit_stop(123, kMaxNameOffset + 1, false, trav);, 
               std::runtime_error) << "TransitStop name offset limit check failed";
