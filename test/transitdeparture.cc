...
TEST(TransitDeparture, TestWriteReadFixed) {
  ...
  // fails.
  EXPECT_THROW(TransitDeparture dep2(kMaxTransitLineId + 100, 222, 333, 444, 555, 9000, 90, 23, false, true), std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, kMaxTripId + 10, 333, 444, 555, 9000, 90, 23, false, true), std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, kMaxTransitRoutes + 10, 444, 555, 9000, 90, 23, false, true), std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, kMaxTransitBlockId + 10, 555, 9000, 90, 23, false, true), std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, kMaxNameOffset + 10, 9000, 90, 23, false, true), std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, 555, kMaxTransitDepartureTime + 10, 90, 23, false, true), std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, 555, 9000, 90, kMaxTransitSchedules + 10, false, true), std::runtime_error);

  // Test that elapsed time is clamped
  TransitDeparture dep1(111, 222, 333, 444, 555, 9000, kMaxTransitElapsedTime + 10, 23, false, true);
  EXPECT_EQ(dep1.elapsed_time() , kMaxTransitElapsedTime) << "TransitDeparture elapsed_time clamping failed";
}
...
TEST(TransitDeparture, TestWriteReadFreq) {
  ...
  // Catch these exceptions - which means test passes. If an exception is not thrown the test
  // fails.
  EXPECT_THROW(TransitDeparture dep(kMaxTransitLineId + 10, 222, 333, 444, 555, 9000, 18000, 600, 90, 23, false, true), 
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, kMaxTripId + 10, 333, 444, 555, 9000, 18000, 600, 90, 23, false, true),
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, kMaxTransitRoutes + 10, 444, 555, 9000, 18000, 600, 90, 23, false, true), 
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, kMaxTransitBlockId + 10, 555, 9000, 18000, 600, 90, 23, false, true), 
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, kMaxNameOffset + 10, 9000, 18000, 600, 90, 23, false, true), 
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, 555, kMaxTransitDepartureTime + 10, 18000, 600, 90, 23, false, true), 
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, 555, 9000, kMaxEndTime + 10, 600, 90, 23, false, true), 
               std::runtime_error);
  EXPECT_THROW(TransitDeparture dep(111, 222, 333, 444, 555, 9000, 18000, kMaxFrequency + 10, 90, 23, false, true), 
               std::runtime_error) << "Frequency limit check failed";
